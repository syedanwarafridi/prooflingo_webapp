...
from .serializers import (
    ...
    BilingoProjectCreateSerializer
)
...
from .bilingo_formatter import formatter

# ----------Handling all the formats of bilingual files----------------------------
from .Bilingual_files import (
    billingual_sengmentation_trados,
    billingual_segmentation_xml,
    billingual_segmentation_mqxliff,
    billingual_segmentation_mxliff,
    billingual_segmentation_sdlppx,
    billingual_segmentation_xliff,
)
        ...
        try:
            ...
            if serializer.is_valid():
                with transaction.atomic():
                    ...
                    team_name = request.user.first_name

                    if Team.objects.filter(owner=request.user, name=team_name).exists():
                        team = Team.objects.get(owner=request.user, name=team_name)
                    else:
                    ...
                    if not TeamMember.objects.filter(user=request.user, team=team).exists():
                        team_member = TeamMember.objects.create(team=team, user=request.user)
                    else:
                    ...
                    if request.FILES.get("source_file"):
                        ...
                        default_storage.save(
                            str(source_file_path), request.FILES["source_file"]
                        )
                        project_instance.source_file = str(
                            source_file_path.relative_to(settings.MEDIA_ROOT)
                        )
                    if request.FILES.get("target_file"):
                        ...
                        default_storage.save(
                            str(target_file_path), request.FILES["target_file"]
                        )
                        project_instance.target_file = str(
                            target_file_path.relative_to(settings.MEDIA_ROOT)
                        )
...
#-------> Bilingual Project Creation API <---------#
class CreateBilingualProject(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request, format=None):
        try:
            if 'bilingual_file' not in request.FILES:
                raise Http404("No file uploaded")

            serializer = BilingoProjectCreateSerializer(data=request.data, context={"request": request})

            if serializer.is_valid():
                with transaction.atomic():
                    project_instance = serializer.save(bilingual=True)

                    project_directory = Path(settings.MEDIA_ROOT) / "project_files" / str(project_instance.id)
                    project_directory.mkdir(parents=True, exist_ok=True)

                    bilingual_file_name = request.FILES['bilingual_file'].name
                    bilingual_file_path = project_directory / bilingual_file_name
                    default_storage.save(str(bilingual_file_path), request.FILES['bilingual_file'])

                    project_instance.source_file = str(bilingual_file_path.relative_to(settings.MEDIA_ROOT))
                    project_instance.target_file = str(bilingual_file_path.relative_to(settings.MEDIA_ROOT))
                    project_instance.created_by = str(request.user.first_nam) + " " + str(request.user.last_name)
                    project_instance.save()
...
#-----------> Content Extraction from Bilingual Files <----------->
import concurrent.futures

class FormatBilingoText(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            file_path = project.source_file

            source_text, target_text = parse_bilingual(file_path)

            # Using ThreadPoolExecutor to run formatter functions concurrently
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future_source = executor.submit(formatter, source_text)
                future_target = executor.submit(formatter, target_text)

                src_txt = future_source.result()
                tgt_txt = future_target.result()

            # Construct the response
            response_data = {
                'Source': [json.loads(src_txt)],
                'Target': [json.loads(tgt_txt)]
            }
    ...
    def get(self, request, project_id, format=None):
        try:
            ...
            source_file_path = os.path.join(
                settings.MEDIA_ROOT, project.source_file.name
            )
            target_file_path = os.path.join(
                settings.MEDIA_ROOT, project.target_file.name
            )
    ...
    def get(self, request, project_id, format=None):
        try:
            ...
            if source_file_ext == ".docx":
                ...
                source_data = extract_text_and_tables_from_pptx_segments(
                    source_file_path
                )
            else:
                return Response(
            ...
            if target_file_ext == ".docx":
                ...
                target_data = extract_text_and_tables_from_pptx_segments(
                    target_file_path
                )
            else:
                return Response(
...
class ProjectPageAPIView(APIView):
    ...
    def post(self, request, project_id, format=None):
        try:
            ...
            if page_no is None:
                return Response(
                    {"error": "Page number is required as a query parameter"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if "Source" not in data or "Target" not in data:
                return Response(
                    {"error": "Source and Target keys are required in request data"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            source_text, target_text, response = process_content(
                data["Source"][page_no],
                data["Target"][page_no],
                history,
                subject_matter,
            ...
            return Response(
                {"error": f"Missing key in request data: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def post(self, request, project_id, format=None):
        try:
            ...
            if not project.target_file:
                return Response(
                    {"error": "Target file not found"}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            updated_file_dir = (
                Path(settings.MEDIA_ROOT) / "updated_target_files" / str(project.id)
            )
            updated_file_dir.mkdir(parents=True, exist_ok=True)

            if target_file_ext == ".docx":
                updated_file_path = (
                    updated_file_dir / f"updated_target_{project.id}.docx"
                )
                process_docx(updated_file_path, comparison_data)
            elif target_file_ext == ".pptx":
                updated_file_path = (
                    updated_file_dir / f"updated_target_{project.id}.pptx"
                )
            ...
            project.updated_target_file = str(
                updated_file_path.relative_to(settings.MEDIA_ROOT)
            )
            project.save()
...
class DeleteProjectsAPIView(APIView):
    ...
    def delete(self, request, format=None):
        ...
        if not isinstance(pks, list):
            return Response(
                {"error": "pks must be a list of primary keys"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"message": f"{count} projects deleted"}, status=status.HTTP_200_OK
            )
        except Exception as e:
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
...
class DeleteProjectAPIView(APIView):
    ...
    def delete(self, request, pk, format=None):
        try:
            ...
            project.delete()
            return Response(
                {"message": f"Project {pk} deleted"}, status=status.HTTP_200_OK
            )
        except Project.DoesNotExist:
            return Response(
                {
...
class UpdateProjectStatusAPIView(APIView):
    ...
    def patch(self, request, pk, format=None):
        try:
            ...
            return Response(
                {
                    "error": "Project not found or you do not have permission to update this project"
                },
                status=status.HTTP_404_NOT_FOUND,
            )

        serializer = ProjectStatusUpdateSerializer(
            project, data={"status": "completed"}, partial=True
        )
        if serializer.is_valid():
            serializer.save()
            return Response(
...
class DuplicateProjectAPIView(APIView):
    ...
    def post(self, request, pk, format=None):
        try:
            ...
            return Response(
                {
                    "error": "Project not found or you do not have permission to duplicate this project"
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        with transaction.atomic():
            new_project = Project.objects.create(
                user=request.user,
                project_name=self.generate_unique_project_name(
...
class AssistantAPIView(APIView):
    def post(self, request, format=None):
        ...
        if not target_text or not query:
            return Response(
                {"error": "Both target_text and query are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def post(self, request, format=None):
        try:
            ...
            if not bilingual_file:
                return Response(
                    {"error": "No bilingual file provided"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if file_extension in [".sdlxliff", ".txlf"]:
                source_file, target_file = billingual_sengmentation_trados(
                    temp_file_path
                )
            elif file_extension == ".xml":
                source_file, target_file = billingual_segmentation_xml(temp_file_path)
            elif file_extension == ".mqxliff":
                source_file, target_file = billingual_segmentation_mqxliff(
                    temp_file_path
                )
            elif file_extension == ".mxliff":
                source_file, target_file = billingual_segmentation_mxliff(
                    temp_file_path
                )
            elif file_extension == ".sdlppx":
                source_file, target_file = billingual_segmentation_sdlppx(
                    temp_file_path
                )
            ...
            source_file = InMemoryUploadedFile(
                source_file,
                None,
                f'{os.path.basename(bilingual_file.name).split(".")[0]}_source.docx',
                "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                source_file.getbuffer().nbytes,
                None,
            )
            target_file = InMemoryUploadedFile(
                target_file,
                None,
                f'{os.path.basename(bilingual_file.name).split(".")[0]}_target.docx',
                "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                target_file.getbuffer().nbytes,
                None,
            )
...
class TextTunningAPI(APIView):
    def post(self, request):
        ...
        if not word:
            return Response(
                {"error": "Word is required"}, status=status.HTTP_400_BAD_REQUEST
            )
